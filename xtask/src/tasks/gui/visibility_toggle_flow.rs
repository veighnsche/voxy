...
pub struct VisibilityToggleFlowArgs {
...
pub fn run(args: VisibilityToggleFlowArgs) -> Result<()> {
    ...
    let app_id = common::make_app_id("visibility-toggle-flow");
    let env = vec![
        (
    ...
    if !status.success() {
        bail!("voxy-app visibility-toggle-flow run exited with non-success status: {status}");
    }

    println!("[xtask] visibility-toggle-flow check passed: {status}");
    Ok(())
}
