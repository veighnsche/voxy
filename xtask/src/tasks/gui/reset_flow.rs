...
pub fn run(args: ResetFlowArgs) -> Result<()> {
    ...
    let app_id = common::make_app_id("reset-flow");
    let env = vec![
        ("VOXY_SMOKE_INJECT_RESET".to_owned(), "1".to_owned()),
        (
    ...
    if !status.success() {
        bail!("voxy-app reset-flow run exited with non-success status: {status}");
    }

    println!("[xtask] reset-flow check passed: {status}");
    Ok(())
}
